  ...
  class CapabilityConfigTest < ActiveSupport::TestCase
    ...
    test "initializes with valid parameters" do
      config = CapabilityConfig.new(
        name: :general_llm,
        model_name: "./vllm/models/qwen3_32B",
        port: 52003,
        max_context: 64000,
        base_url: "LLM_URL"
      )
      ...
      assert_equal "./vllm/models/qwen3_32B", config.model_name
    ...
    speed_profile :fast
    test "validates model_name cannot be empty" do
      error = assert_raises(ArgumentError) do
        CapabilityConfig.new(
          name: :test,
          model_name: "",
      ...
      assert_match(/model_name cannot be empty/, error.message)
    end
    ...
    test "to_h serializes all attributes" do
      ...
      assert_equal "test_model", hash[:model_name]
